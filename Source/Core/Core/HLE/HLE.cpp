...
constexpr std::array<Hook, 24> os_patches{{
    ...
    // MH Tri (RMHE08) VR camera injection — installed by MHTriVR::TryInstall at a fixed address.
    {"MHTriVRCamPost",               MHTriVR::CamPostHook,                  HookType::Start,   HookFlag::Fixed}
}};
// clang-format on
...
void PatchFixedFunctions(Core::System& system)
{
  ...
  // MH Tri (RMHE08) VR camera injection. No-op for any other game.
  MHTriVR::TryInstall(system);
}
